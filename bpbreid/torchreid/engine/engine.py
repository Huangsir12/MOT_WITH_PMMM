...
class Engine(object):
    ...
    def __init__(self, config, datamanager, writer, engine_state, test_only=False, use_gpu=True, save_model_flag=False, detailed_ranking=False):
        self.config = config
    ...
    def _evaluate(
        ...
        if load_features:
            ...
            if osp.exists(gallery_features_path):
                gf = torch.load(gallery_features_path)
                gallery_metadata_path = gallery_features_path.replace('gallery_features', 'gallery_metadata')
                g_pids, g_camids, g_anns = torch.load(osp.join(features_dir, 'gallery_metadata_' + dataset_name + '.pt'))
                print("Features loaded successfully.")
            else:
                print("Saved features not found. Extracting features...")
                load_features = False  
        ...
        if save_features:
            ...
            torch.save((g_pids, g_camids, g_anns), osp.join(features_dir, 'gallery_metadata_' + dataset_name + '.pt'))
