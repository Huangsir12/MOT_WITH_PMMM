...
def get_default_config():
    ...
    cfg.test.visrank_count = 2 # number of top-k ranks to plot
    cfg.test.visrank_q_idx_list = [0, 1]  # list of ids of queries for which we want to plot topk rank. If len(visrank_q_idx_list) < visrank_count, remaining ids will be random
    ...
    cfg.inference.num_classes = 0
    cfg.inference.dataset_folder = ''
    cfg.inference.dataset_name = ''
    cfg.inference.save_features = False
    cfg.inference.output_folder = ''
    cfg.inference.ranks = [1, 5, 10, 20]
    cfg.inference.load_gallery_features = False
    cfg.inference.gallery_features_path = ''
...
def engine_run_kwargs(cfg):
    return {
        ...
        'load_features': cfg.test.load_features,
        'gallery_features_path': cfg.test.gallery_features_path,
        'save_features': cfg.test.save_features
    }

def inference_run_kwargs(cfg):
    return {
        'save_gallery_features': cfg.inference.save_features,
        'load_gallery_features': cfg.inference.load_gallery_features,
        'gallery_features_path': cfg.inference.gallery_features_path,
        'save_dir': cfg.data.save_dir,
        'normalize_feature': cfg.test.normalize_feature,
        'dist_metric': cfg.test.dist_metric,
        'rerank': cfg.test.rerank,
