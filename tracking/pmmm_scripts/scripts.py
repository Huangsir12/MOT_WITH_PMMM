...
advented_consecutive_frames = 12           # 出现持续帧数
gallery_frame_interval = 6                 # 放入gallery集的同ids间隔帧数
query_frame_interval = 3                   # 放入query集的同ids间隔帧数
max_memory_length = 200                    # 最大记忆帧长，定时清除间隔超该数的clops
min_memory_length = 150                    # 最小记忆帧长，出现abnormal ID时，将最近150帧，一定的间隔的clop记忆
...
def add_to_gallery_dataset(frame, clops_output_dir, gallery_dataset_dir, abnormal_removed):
    havend_add_ids = []
    for track_id in abnormal_removed:
        file_name = []
        for file in os.listdir(clops_output_dir):
            frame_id  = int(file.split("_")[1])
            if file.endswith(f"ID_{track_id}.jpg") and (frame_id % gallery_frame_interval  == 0) and (frame - frame_id < min_memory_length):
...
def clear_gallery_dataset_frame(dataset_dir, frame_id):
    for file in os.listdir(dataset_dir):
        if frame_id - int(file.split("_")[1]) > max_memory_length:
            file_path = os.path.join(dataset_dir, file)
            if os.path.isfile(file_path):
                os.remove(file_path)
    g_pids = list(set([int(file.split("_")[3].split(".")[0]) for file in os.listdir(dataset_dir)]))
    return g_pids

def clear_gallery_dataset(dataset_dir, frame_id, pids, track_ids):
    ...
    for track_id in track_ids:
        if track_id in pids:
            pids.remove(track_id)
            clear_ids.append(track_id)
...
def renew_track_ids(track_results_ids, matched):
    ...
    for frame_track_ids in track_results_ids:
        for track_id in frame_track_ids:
            if track_id in updated_dict and updated_dict[track_id] in frame_track_ids:
                updated_dict.pop(track_id)
