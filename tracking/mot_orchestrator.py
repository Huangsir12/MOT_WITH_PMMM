...
from tracking.video_renderer import VideoRenderer
...
class MOTOrchestrator:
    ...
    def __init__(self, command):
        ...
        if command == 'link':
            ...
            self.video_renderer = VideoRenderer(db_manager=self.db_manager)
        if command == 'render-videos':
            self.video_renderer = VideoRenderer(db_manager=self.db_manager)
        if command == 'analyze-features':
    ...
    def link_tracklets(self, scenario_name: str, tracking_batch: int,
        ...
        """
        Step 4: Link tracklets to form person trajectories with temporal constraints
        ...
        Args:
            ...
            use_temporal: Whether to use temporal distance
    ...
    def render_videos(self,
        ...
        """
        Step 8: Render videos with linked person IDs (global track_id)
        ...
        """
        print("=" * 80)
        print(f"STEP 8: Rendering videos for '{scenario_name}' batch {tracking_batch}")
        if linking_batch:
            print(f"Linking batch: {linking_batch}")
        print("=" * 80)
        ...
        # Render videos
        result = self.video_renderer.render_scenario(
            scenario_name=scenario_name,
            tracking_batch=tracking_batch,
            linking_batch=linking_batch,
            output_dir=self.results_base_path,
...
def main():
    ...
    parser.add_argument(
        'command',
        choices=['register', 'process', 'query', 'link', 'query-persons', 'analyze-features', 'query-features', 'render-videos', 'full'],
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Only link tracklets from same camera (default: True)'
    )

    args = parser.parse_args()

    # Initialize orchestrator
    orchestrator = MOTOrchestrator(args.command)
    ...
    if args.command == 'register':
        ...
        if link_result['person_count'] > 0 and args.render_videos:
            orchestrator.render_videos(
                scenario_name=args.scenario,
                tracking_batch=args.batch,
                linking_batch=args.linking_batch,
                camera_name=args.camera,
